...
class CDRClusterer:
    ...
    def __init__(self, pose):
        self.file_dir = os.path.split(os.path.abspath(__file__))[0]
        self.pose = pose
        self.dihedrals = None
        self.regions = {
            "L1":['L', 24, 42],
    ...
    def set_dihedrals_from_pose(self, start, end, chain):
        self.dihedrals = defaultdict()
        angles = ['phi', 'psi', 'omega']
        for angle in angles: self.dihedrals[angle] = []
        
        rosetta_start = self.pose.pdb_info().pdb2pose(chain, start)
        rosetta_end = self.pose.pdb_info().pdb2pose(chain, end)
        for rosetta_num in range(rosetta_start, rosetta_end+1):
            
            self.dihedrals['phi'].append(self.pose.phi(rosetta_num))
            self.dihedrals['psi'].append(self.pose.psi(rosetta_num))
            self.dihedrals['omega'].append(self.pose.omega(rosetta_num-1))
    
    def set_dihedrals_from_cdr(self, cdr_name, chain):
        start = self.regions[cdr_name][1]
        end = self.regions[cdr_name][2]
        self.set_dihedrals_from_pose(start, end, chain)
    ...
    def get_sequence(self, cdr_name):
        seq = ""

        resnum_start = self.pose.pdb_info().pdb2pose(self.regions[cdr_name][0], self.regions[cdr_name][1])
        resnum_end = self.pose.pdb_info().pdb2pose(self.regions[cdr_name][0], self.regions[cdr_name][2])
        full_seq = self.pose.sequence()


        seq = full_seq[resnum_start-1:resnum_end]

        return seq

    def get_fullcluster(self, cdr_name, chain = None, region = False):
        """
