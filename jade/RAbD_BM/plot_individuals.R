...
for (rec_type in c("length", "cluster")){
    ...
    plot_field_grid = function(p, plot_name, grid = NULL){
        ...
        pdf(file = outpath, height=15, width=18, res=300, units="in")
        print(p)
        dev.off()
    }
    ...
    p <- ggplot(data=exp_data, na.rm=T) +
        geom_bar(position="dodge", stat="identity", aes(x=exp_group, y= top_rec)) +
        ggtitle(paste("GraftDesign Native", rec_type, "Recovery")) +
        theme_bw() +
        xlab("")+
        ylab("% Recovery")

    plot_field_grid(p, "ind_rec_by_exp", ~h3_present)


    #Plot Experimental RR
    p <- ggplot(data=exp_data, na.rm=T) +
        geom_bar(position="dodge", stat="identity", aes(x=exp, y= top_rr)) +
    ...
    plot_field_grid(p, "ind_rr_by_exp", ~h3_present)
    ...
    p <- ggplot(data=cdr_data, na.rm = T) +
        geom_bar(position="dodge", stat="identity", aes(x=exp, y = top_rec)) +
        ggtitle(paste("GraftDesign Native", rec_type, "Recovery")) +
        theme_bw() +
        xlab("") +
        ylab("% Recovery")

    plot_field_wrap(p, "ind_rec_by_cdr", ~CDR)
    ...
    plot_field_wrap(p, "ind_rr_by_cdr", ~CDR)
